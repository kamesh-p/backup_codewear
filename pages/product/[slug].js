...
export default function Page({ addcart }) {
  ...
  const [pin, setPin] = useState(""); // Initialize with an empty string
  ...
  const availability = async () => {
    ...
    if (pinscode.includes(Number(pin))) {
      // Convert pin to a number for comparison
      setService(true);
    } else {
      setService(false);
    }

    // console.log("pin", pinscode);
  };
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                <button
                  onClick={() => color("white")} // Pass the color as an argument
                  className="border-2 border-gray-300 rounded-full w-6 h-6 focus:outline-none"
                ></button>
                <button
                  onClick={() => color("gray")} // Pass the color as an argument
                  className="border-2 border-gray-300 ml-1 bg-gray-700 rounded-full w-6 h-6 focus:outline-none"
                ></button>
                <button
                  onClick={() => color("blue")} // Pass the color as an argument
            ...
            {!service && service !== null && (
              <h2 className="mx-5 mt-2 font-semibold text-red-500">
                sorry, Your are out of our services
              </h2>
            )}
          </div>
        </div>
